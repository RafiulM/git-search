...
class TwitterService:
    ...
    def __init__(self):
        ...
        logger.debug(
            f"  TWITTER_CONSUMER_KEY: {'✅ Set' if self.consumer_key else '❌ Missing'}"
        )
        logger.debug(
            f"  TWITTER_CONSUMER_SECRET: {'✅ Set' if self.consumer_secret else '❌ Missing'}"
        )
        logger.debug(
            f"  TWITTER_ACCESS_TOKEN: {'✅ Set' if self.access_token else '❌ Missing'}"
        )
        logger.debug(
            f"  TWITTER_ACCESS_TOKEN_SECRET: {'✅ Set' if self.access_token_secret else '❌ Missing'}"
        )
        logger.debug(
            f"  TWITTER_BEARER_TOKEN: {'✅ Set' if self.bearer_token else '❌ Missing'}"
        )
        ...
        if not all(required_creds):
            logger.warning(
                "Twitter API credentials not fully configured. Twitter posting will be disabled."
            )
            self.client = None
            self.api = None
        else:
            try:
                ...
                try:
                    ...
                    if me.data:
                        logger.info(
                            f"✅ Twitter service initialized successfully for @{me.data.username}"
                        )
                    else:
                        logger.warning(
                            "⚠️ Twitter authentication succeeded but couldn't get user info"
                        )
                    ...
                    if "401" in error_msg or "Unauthorized" in error_msg:
                        logger.error(
                            "🔑 401 Unauthorized Error - This typically means:"
                        )
                        logger.error(
                            "   1. Invalid API credentials (check if they're correct)"
                        )
        ...
        try:
            ...
            if description and description.strip():
                ...
                if clean_desc not in [d for d in generic_descriptions if d]:
                    ...
                    base_length = (
                        len(main_tweet_parts[0]) + 10
                    )  # thread indicator ~5 + buffer ~5
                    ...
                    if len(clean_desc) > available_for_desc:
                        ...
                        if (
                            best_ending > available_for_desc * 0.7
                        ):  # If we found a good ending point
            ...
            if len(main_tweet) > 280:
                logger.warning(
                    f"Main tweet too long ({len(main_tweet)} chars), trimming..."
                )
                ...
                if len(main_tweet_parts) > 2 and description:
                    ...
                    if remaining_for_desc > 20:
                        ...
                        main_tweet = "".join(
                            [main_tweet_parts[0], main_tweet_parts[-1]]
                        )

            if len(reply_tweet) > 280:
                logger.warning(
                    f"Reply tweet too long ({len(reply_tweet)} chars), trimming..."
                )
            ...
            logger.info(
                f"Generated thread - Main: {len(main_tweet)} chars, Reply: {len(reply_tweet)} chars"
            )

            return {"main_tweet": main_tweet, "reply_tweet": reply_tweet}

        except Exception as e:
            logger.error(
                f"Error generating thread for repository {repo_info.get('name', 'unknown')}: {str(e)}"
            )
    ...
    def generate_repository_tweet(self, repo_info: Dict[str, Any]) -> str:
        ...
        main = thread["main_tweet"]  # Remove thread indicator
    ...
    def upload_media(
        self, media_path: str, alt_text: Optional[str] = None
    ) -> Optional[str]:
        ...
        try:
            ...
            if not self.api:
                logger.error("Twitter API is not configured")
                return None
    ...
    def download_and_upload_media(
        ...
        try:
            ...
            logger.error(
                f"Failed to download and upload media from {image_url}: {str(e)}"
            )
            return None

    async def post_tweet(
        self, tweet_text: str, media_ids: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.client:
                logger.error("Twitter client is not configured")
                return {
                    "success": False,
                    "error": "Twitter client is not configured",
                    "tweet_id": None,
                    "tweet_url": None,
                }
            ...
            if response.data:
                ...
                tweet_url = (
                    f"https://twitter.com/intent/{twitter_username}/status/{tweet_id}"
                )
    ...
    async def post_thread(
        self, main_tweet: str, reply_tweet: str, media_ids: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"Posting thread - Main: {main_tweet[:50]}... Reply: {reply_tweet[:50]}..."
            )
            if media_ids:
                logger.info(
                    f"Including {len(media_ids)} media attachments in main tweet"
                )

            # Post the main tweet with optional media
            if media_ids:
                main_response = self.client.create_tweet(
                    text=main_tweet, media_ids=media_ids
                )
            else:
            ...
            reply_response = self.client.create_tweet(
                text=reply_tweet, in_reply_to_tweet_id=main_tweet_id
            )

            if reply_response.data:
                reply_tweet_id = reply_response.data["id"]
                thread_url = (
                    f"https://twitter.com/intent/tweet?tweet_id={main_tweet_id}"
                )

                logger.info(
                    f"Thread posted successfully - Main: {main_tweet_id}, Reply: {reply_tweet_id}"
                )
    ...
    def validate_repository_description(
    ...
    async def post_repository_tweet(
        self, repo_info: Dict[str, Any], include_media: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Repository {repo_info.get('name')} passed description validation"
            )
            ...
            if include_media and repo_info.get("readme_image_url"):
                try:
                    ...
                    media_id = self.download_and_upload_media(
                        repo_info["readme_image_url"], alt_text
                    )
                    ...
                    logger.error(
                        f"Error processing media for repository {repo_info.get('name', 'unknown')}: {str(media_error)}"
                    )
            ...
            error_msg = str(e)
            logger.error(
                f"Failed to post repository thread for {repo_info.get('name', 'unknown')}: {error_msg}"
            )
            return {
                "success": False,
                "error": error_msg,
...
class TwitterServiceSingleton:
    ...
    def __getattr__(self, name):
        if self._service is None:
            ...
            logger.info(
                "🔄 Initializing Twitter service with fresh environment variables..."
            )
