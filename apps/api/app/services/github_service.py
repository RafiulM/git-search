...
class GitHubService:
    ...
    def _load_config_from_env(self) -> GitHubConfig:
        ...
        if not token:
            raise ValueError("GitHub token is required. Set GITHUB_TOKEN or GITHUB_ACCESS_TOKEN environment variable")
    ...
    async def fork_repository_internal(
        ...
        try:
            ...
            logger.debug(f"Successfully retrieved source repository: {source_gh_repo.full_name}")
            ...
            if organization:
                ...
                forked_repo = source_gh_repo.create_fork(org, name=fork_name)
            else:
                # Fork to user account
                logger.debug("Forking to user account")
                forked_repo = source_gh_repo.create_fork(name=fork_name)
            ...
            error_msg = f"GitHub API error: {str(e) if e else 'Unknown GitHub API error'}"
            status_code = getattr(e, 'status', None)
            
            if status_code == 422:
                # Repository already forked or validation error
                error_msg = f"Fork already exists or validation error: {str(e)}"
                logger.warning(f"Repository {source_repo.full_name} already forked or validation error: {str(e)}")
            elif status_code == 403:
                # Forbidden - insufficient permissions or rate limited
                error_msg = f"Insufficient permissions or rate limited: {str(e)}"
                logger.error(f"Insufficient permissions to fork {source_repo.full_name}: {str(e)}")
            elif status_code == 404:
                # Repository not found or private
                error_msg = f"Repository not found or private: {str(e)}"
                logger.error(f"Repository {source_repo.full_name} not found or private: {str(e)}")
            else:
                if status_code:
                    error_msg += f" (Status: {status_code})"
                if hasattr(e, 'data') and e.data:
                    error_msg += f" (Details: {e.data})"
                logger.error(f"GitHub API error forking repository {source_repo.full_name}: {error_msg}")
            ...
            logger.error(f"Error forking repository {source_repo.full_name}: {error_msg}")
        ...
        try:
            ...
            if fork_result.success:
                return {
                    ...
                    "fork_full_name": fork_result.metadata.get("fork_full_name") if fork_result.metadata else None,
                    "default_branch": fork_result.metadata.get("default_branch") if fork_result.metadata else None
                }
            else:
                error_msg = fork_result.error if fork_result.error else "Unknown fork failure"
    ...
    async def clone_repository(
        ...
        try:
            ...
            if self.config.git_user_name:
                git_repo.config_writer().set_value("user", "name", self.config.git_user_name).release()
            if self.config.git_user_email:
                git_repo.config_writer().set_value("user", "email", self.config.git_user_email).release()
    ...
    async def apply_file_operations(
        ...
        for operation in file_operations:
            ...
            try:
                if operation.action == GitCommitAction.CREATE or operation.action == GitCommitAction.UPDATE:
                ...
                logger.error(f"Error applying {operation.action} to {operation.path}: {str(e)}")
                raise
    ...
    async def commit_changes(
        ...
        try:
            ...
            file_results = await self.apply_file_operations(repo_path, commit_info.files)
            ...
            # Make commit
            commit_args = {'message': commit_info.message}
            if actor:
                commit_args['author'] = actor
                commit_args['committer'] = actor
            
            commit = git_repo.index.commit(**commit_args)
            ...
            return GitHubCommitResult(
                ...
                files_modified=file_results["created"] + file_results["updated"] + file_results["deleted"],
                processing_time=time.time() - start_time,
                metadata={
        ...
        try:
            ...
            if not use_ssh and self.config.token:
                ...
                if remote_url.startswith("https://github.com/"):
                    remote_url = remote_url.replace("https://github.com/", f"https://{self.config.token}@github.com/")
    ...
    async def fork_and_modify(
        ...
        try:
            logger.info(f"Starting fork and modify workflow for {request.source_repo.full_name}")
            ...
            clone_config = request.clone_config or CloneConfig(repo=fork_result.target_repo)
            ...
            return GitHubForkAndModifyResult(
                ...
                metadata={
                    "workflow_steps": [
                        ...
                        "push" if push_result and push_result.success else "push_skipped_or_failed",
                        "cleanup" if request.cleanup_after else "no_cleanup"
                    ],
                    "total_files_modified": len(commit_result.files_modified) if commit_result.success else 0
                }
            )
        ...
        try:
            ...
            return {
                ...
                "git_config": {
                    ...
                    "configured": bool(self.config.git_user_name and self.config.git_user_email)
                }
            }
