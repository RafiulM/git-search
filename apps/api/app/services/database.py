...
class DatabaseService:
    ...
    async def create_twitter_posting(
        self, posting_data: TwitterPostingInsert
    ) -> TwitterPosting:
        """Create a new Twitter posting job"""
        try:
            ...
            # Map required fields
            if posting_data.job_name:
                data["job_name"] = posting_data.job_name
            else:
                raise ValueError("job_name is required")

            if posting_data.total_repositories is not None:
                data["total_repositories"] = posting_data.total_repositories
            else:
                raise ValueError("total_repositories is required")

            # Map fields with defaults
            data["processed_repositories"] = posting_data.processed_repositories
            data["successful_posts"] = posting_data.successful_posts
            data["failed_posts"] = posting_data.failed_posts
            data["rate_limited_posts"] = posting_data.rate_limited_posts
            data["status"] = posting_data.status

            # Handle lists as JSON
            if posting_data.repository_ids is not None:
                data["repository_ids"] = json.dumps(posting_data.repository_ids)
            else:
                data["repository_ids"] = json.dumps([])

            if posting_data.posted_tweet_urls is not None:
                data["posted_tweet_urls"] = json.dumps(posting_data.posted_tweet_urls)
            else:
                data["posted_tweet_urls"] = json.dumps([])

            # Optional fields
            if posting_data.error_message is not None:
                data["error_message"] = posting_data.error_message
            if posting_data.started_at is not None:
                if isinstance(posting_data.started_at, datetime):
                    data["started_at"] = posting_data.started_at.isoformat()
                else:
                    data["started_at"] = posting_data.started_at
            if posting_data.completed_at is not None:
                if isinstance(posting_data.completed_at, datetime):
                    data["completed_at"] = posting_data.completed_at.isoformat()
                else:
                    data["completed_at"] = posting_data.completed_at

            # Handle metadata as JSON
            if posting_data.metadata is not None:
                data["metadata"] = json.dumps(
                    posting_data.metadata, cls=DateTimeEncoder
                )

            result = self.client.table("twitter_posting").insert(data).execute()
            ...
            if result.data:
                ...
                if isinstance(row_data.get("repository_ids"), str):
                    try:
                        ...
                        row_data["repository_ids"] = []
                if isinstance(row_data.get("posted_tweet_urls"), str):
                    try:
                        row_data["posted_tweet_urls"] = json.loads(
                            row_data["posted_tweet_urls"]
                        )
                    except json.JSONDecodeError:
                        row_data["posted_tweet_urls"] = []
                if isinstance(row_data.get("metadata"), str):
                ...
                return TwitterPosting(**row_data)
            else:
                raise Exception("Failed to create Twitter posting")

        except Exception as e:
            raise Exception(f"Database error creating Twitter posting: {str(e)}")

    async def get_twitter_posting(self, posting_id: UUID) -> Optional[TwitterPosting]:
        """Get Twitter posting by ID"""
        try:
            result = (
                self.client.table("twitter_posting")
                .select("*")
            ...
            raise Exception(f"Database error getting Twitter posting: {str(e)}")

    async def update_twitter_posting(
        self, posting_id: UUID, posting_data: TwitterPostingUpdate
    ) -> Optional[TwitterPosting]:
        """Update Twitter posting"""
        try:
            data = {}

            # Helper function to get value from update data
            def get_value(field_name):
                return getattr(posting_data, field_name, None)

            # Map fields that can be updated
            if get_value("job_name") is not None:
                data["job_name"] = get_value("job_name")
            if get_value("total_repositories") is not None:
                data["total_repositories"] = get_value("total_repositories")
            if get_value("processed_repositories") is not None:
                data["processed_repositories"] = get_value("processed_repositories")
            if get_value("successful_posts") is not None:
                data["successful_posts"] = get_value("successful_posts")
            if get_value("failed_posts") is not None:
                data["failed_posts"] = get_value("failed_posts")
            if get_value("rate_limited_posts") is not None:
                data["rate_limited_posts"] = get_value("rate_limited_posts")
            if get_value("status") is not None:
            ...
            if get_value("repository_ids") is not None:
                data["repository_ids"] = json.dumps(get_value("repository_ids"))
            if get_value("posted_tweet_urls") is not None:
                data["posted_tweet_urls"] = json.dumps(get_value("posted_tweet_urls"))
            ...
            if not data:
                return await self.get_twitter_posting(posting_id)
            ...
            if result.data:
                # Parse JSON strings back to appropriate types for Pydantic model
                row_data = result.data[0]
                if isinstance(row_data.get("repository_ids"), str):
                    try:
                        row_data["repository_ids"] = json.loads(
            ...
            raise Exception(f"Database error updating Twitter posting: {str(e)}")
